...
{
  // This implementation assumes AMD64.
  // Address can be represented by lower 48 bits
...
public:
  ...
  std::pair<T*, StampType> load(std::memory_order order = std::memory_order_seq_cst) const noexcept
  {
    auto data = mData.load(order);
    return unzip(data);
  }
  std::pair<T*, StampType> load(std::memory_order order = std::memory_order_seq_cst) const volatile noexcept
  {
    auto data = mData.load(order);
    return unzip(data);
  }
